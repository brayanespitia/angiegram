      ...
      Publication.find({ user: { $in: follows_clean } })
        ...
        .paginate(page, itemsPerPage, (err, publications, total) => {
          if (err)
            return res
              .status(500)
              .send({ message: "Error devolver publicaciones" });
...
function deletePublication(req, res) {
  ...
  Publication.find({ user: req.user.sub, _id: publicationId }).remove((err) => {
    if (err)
      return res.status(500).send({ message: "Error eliminar publicaciones" });

    //if (!publicationRemoved) return res.status(404).send({ message: "No elimino la publicación" });
